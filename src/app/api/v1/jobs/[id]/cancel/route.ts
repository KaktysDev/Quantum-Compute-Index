...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: job, error } = await admin.from("jobs").select("*, quotes!job_id(total)").eq("id", id).eq("organization_id", principal.organizationId).maybeSingle();
    ...
    if (job.status === "cancellation_requested") return NextResponse.json(slimJobForOwner(job as Record<string, unknown>));
    if (["submitted", "processing"].includes(job.status) && job.provider_job_id) {
      // Clearing the deadline keeps the poller's timeout-failover branch from
      // resurrecting a job the user explicitly cancelled.
      const { data: changed, error: updateError } = await admin.from("jobs").update({ status: "cancellation_requested", timeout_failover_pending: false, execution_deadline_at: null, updated_at: new Date().toISOString() }).eq("id", id).eq("status", job.status).select("id").maybeSingle();
      ...
      return NextResponse.json(slimJobForOwner({ ...job, status: "cancellation_requested" } as Record<string, unknown>), { status: 202 });
    }
    ...
    return NextResponse.json(slimJobForOwner({ ...job, status: "cancelled", completed_at: completedAt } as Record<string, unknown>));
  } catch (error) {
