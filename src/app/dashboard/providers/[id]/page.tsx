...
export default async function ProviderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="console-page provider-detail-page">
    ...
    <header className="provider-detail-hero"><div className="provider-mark large">{backend.displayName.slice(0, 2).toUpperCase()}</div><div><p>{backend.provider} / {backend.kind}</p><h1>{backend.displayName}</h1><span>{backend.description}</span></div><div><span className={backend.available ? "catalog-live" : "catalog-standby"}><i />{backend.available ? "Connected" : "Credential required"}</span><Link href={`/dashboard/github/deploy?target=${backend.id}`}>Deploy to target <ArrowRight size={13} /></Link></div></header>
