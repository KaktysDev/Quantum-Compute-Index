...
export default function ProviderKeyForm() {
  ...
  const [multi, setMulti] = useState<Record<string, Record<string, string>>>({});
  ...
  async function save(p: ProviderStatus) {
    ...
    if (p.fields && p.fields.length > 0) {
      const vals = multi[p.id] ?? {};
      const missing = p.fields.filter((f) => !(vals[f.key] ?? "").trim());
      if (missing.length > 0) {
        setError(`Fill in: ${missing.map((m) => m.label).join(", ")}`);
        return;
      }
      const obj: Record<string, string> = {};
      for (const f of p.fields) obj[f.key] = (vals[f.key] ?? "").trim();
      apiKey = JSON.stringify(obj);
    } else {
      apiKey = (drafts[p.id] ?? "").trim();
      if (apiKey.length < 4) {
    ...
    try {
      ...
      setDrafts((d) => ({ ...d, [p.id]: "" }));
      setMulti((m) => ({ ...m, [p.id]: {} }));
      setNotice(`Saved ${p.name}. ${p.testable ? "Use “Test connection” to verify." : "The next 9:30 AM ET refresh will include it."}`);
      await load();
    } catch (e) {
  ...
  async function test(id: string) {
    ...
    try {
      ...
      setTests((t) => ({
        ...t,
        [id]: { ok: !!j.ok, message: j.message || j.error || "No response", details: j.details },
      }));
    } catch (e) {
      setTests((t) => ({
        ...t,
  ...
  async function remove(id: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/keys?provider=${encodeURIComponent(id)}`, { method: "DELETE" });
      const j = await res.json().catch(() => ({}));
  ...
  const inputClass =
    "tabular w-full rounded-lg border border-white/10 bg-black/40 px-3.5 py-2 text-sm text-white outline-none placeholder:text-[var(--muted)] focus:border-white/40";
          ...
          <div key={p.id} className="glass rounded-2xl p-5">
            ...
            {p.fields && p.fields.length > 0 ? (
              <div className="mt-4 grid gap-2 sm:grid-cols-3">
                {p.fields.map((f) => (
                  <div key={f.key}>
                    <label className="mono-label normal-case tracking-normal">{f.label}</label>
                    <input
                      type={f.type === "password" ? "password" : "text"}
                      autoComplete="off"
                      placeholder={p.configured ? "Replace…" : f.placeholder || f.label}
                      value={multi[p.id]?.[f.key] ?? ""}
                      onChange={(e) =>
                        setMulti((m) => ({
                          ...m,
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              >
                {busy[p.id] ? "Working…" : p.configured ? "Update" : "Save key"}
              </button>
              {p.configured && p.testable && (
                <button
                  onClick={() => test(p.id)}
                  disabled={busy[p.id]}
              ...
              <div
                className={`mt-3 rounded-lg border px-4 py-2.5 text-sm ${
                  t.ok
                    ...
                    : "border-white/15 bg-white/[0.03] text-[var(--muted)]"
                }`}
              >
                <p>{t.ok ? "✓ " : "• "}{t.message}</p>
                {t.details && t.details.length > 0 && (
                  <ul className="tabular mt-2 space-y-0.5 text-xs text-[var(--muted)]">
                    {t.details.map((d, i) => (
                      <li key={i}>— {d}</li>
                    ))}
                  </ul>
