...
/** Landing page order — same five names, filled from the real encoding trace. */
...
export function overlayExecute(stages: EncodingStage[] | undefined, jobStatus?: string): EncodingStage[] {
  ...
  const byId = new Map(base.map((stage) => [stage.id, stage]));
  ...
  return LANDING_ORDER.map((id) => byId.get(id)).filter((stage): stage is EncodingStage => Boolean(stage));
}
